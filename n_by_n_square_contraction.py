...
def contract_n_by_n_network(tensor_list: list[np.ndarray], n: int, compress: bool = False,
    ...
    def modular_2x_1xn_contraction(bottom_1xn: list, top_1xn: list):
        # Contract each pair of tensors in bottom_1xn and top_1xn
    ...
    def final_contraction(bottom_1xn: list, top_1xn: list):
        contracted_1xn = []
        for i, _ in enumerate(bottom_1xn): # contract into final 1xn
            bottom = bottom_1xn[i]
            top = top_1xn[i]
            shape = top.shape
            if(i == 0): # left
                top = top.reshape(1, shape[0], shape[1])
            elif(i == len(bottom_1xn) - 1): # right
    ...
    relative_error = total_error / final_contracted_value
...
if __name__ == "__main__":
    ...
    result_compressed, terr, rerr = contract_n_by_n_network(tensor_list, n, compress=True, dim=2)
    result_compressed_2, terr2, rerr2 = contract_n_by_n_network(tensor_list, n, compress=True, lose_dim=6)
    result_compressed_3, terr3, rerr3 = contract_n_by_n_network(tensor_list, n, compress=True, comp_percent=.34)
