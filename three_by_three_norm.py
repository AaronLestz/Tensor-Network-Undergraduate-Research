...
# Defining proper direction order as (left, up right, down, out)
...
def contract_outer_indices_with_self(tensor_list: list[np.ndarray]) -> list[np.ndarray]:
    """
    Contracts a 3x3x1 tensor network's outer indices, where 
    each tensor has one outer index, with themselves by contracting
    over those indices with a copy of itself.
    For the purpose of preparing for 3x3 contraction when computing norm.
    """
    contracted_list = []
    for tensor in tensor_list:
        order = len(tensor.shape)
        if(order == 3): # outer is always last index, so order of others doesn't matter here
            contracted = np.einsum("abo, ABo -> aAbB", tensor, tensor.conj())
            contracted = contracted.reshape(
                tensor.shape[0] * tensor.shape[0],
                tensor.shape[1] * tensor.shape[1])
        elif(order == 4):
            contracted = np.einsum("abco, ABCo -> aAbBcC", tensor, tensor.conj())
            contracted = contracted.reshape(
                tensor.shape[0] * tensor.shape[0],
                tensor.shape[1] * tensor.shape[1],
                tensor.shape[2] * tensor.shape[2])
        else:
            contracted = np.einsum("abcdo, ABCDo -> aAbBcCdD", tensor, tensor.conj())
            contracted = contracted.reshape(
                tensor.shape[0] * tensor.shape[0],
                tensor.shape[1] * tensor.shape[1],
                tensor.shape[2] * tensor.shape[2],
        ...
        contracted_list.append(contracted)
...
def compute_3x3_norm(tensor_list: list):
    """
    Computes norm by contracting a tensor network with itself.
    tensor_list is in the order as specified in creation (above).
    """
    squared_list = contract_outer_indices_with_self(tensor_list)
    norm_squared = contract_three_by_three_network(squared_list)
...
if __name__ == "__main__":
    ...
    for tensor in tensor_list:
        print(tensor)
    norm = compute_3x3_norm(tensor_list)
    print("Norm of 3x3: ", norm)